...
div
  ...
  b-alert(show)
    | {{ $t('stopwatch.earlyExperiment') }}

  b-input-group(size="lg")
    b-input(v-model="label" :placeholder="$t('stopwatch.whatAreYouWorkingOn')")
  ...
  div(v-if="loading")
    span.text-muted.center.aw-loading {{ $t('common.loading') }}
  div.row(v-else)
    div.col-md-12
      h3 {{ $t('stopwatch.runningTitle') }}
      div(v-if="runningTimers.length > 0")
        div(v-for="e in runningTimers" :key="e.id")
          ...
          hr(style="margin: 0")
      div(v-else)
        span(style="color: #555") {{ $t('stopwatch.noStopwatchRunning') }}
        hr
