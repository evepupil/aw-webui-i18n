...
div
  ...
  b-alert(show variant="warning")
    i18n(path="graph.prInfo" tag="span")
      template(#link)
        a(href="https://github.com/ActivityWatch/aw-webui/pull/365") aw-webui#365

  | {{ $t('graph.description') }}

  b-alert.mt-2(style="warning" show)
    | {{ $t('graph.earlyDevelopment') }}
  ...
  div.d-flex.mt-1
    span.mr-auto.small(style="color: #666") {{ $t('graph.hostname', { hostname: queryOptions.hostname }) }}
  ...
  div(v-if="events != null")
    ...
    div
      | {{ $t('graph.didntFind') }}
      br
      | {{ $t('graph.addWeek') }} #[b-button(size="sm" variant="outline-dark" @click="start = start.subtract(1, 'week'); search()") {{ $t('graph.plusOneWeek') }}]
</template>
