...
b-modal(v-if="event && event.id", :id="'edit-modal-' + event.id", ref="eventEditModal", :title="$t('eventEditor.title')", centered, hide-footer)
  div(v-if="!editedEvent")
    | {{ $t('eventEditor.loadingEvent') }}

  div(v-else)
    table(style="width: 100%")
      tr
        th {{ $t('eventEditor.bucketHeader') }}
        td {{ bucket_id }}
      tr
        th {{ $t('eventEditor.idHeader') }}
        td {{ event.id }}
      tr
        th {{ $t('eventEditor.startHeader') }}
        datetime(type="datetime" v-model="start")
      tr
        th {{ $t('eventEditor.endHeader') }}
        datetime(type="datetime" v-model="end")
      tr
        th {{ $t('eventEditor.durationHeader') }}
    ...
    table(style="width: 100%")
      tr
        th {{ $t('eventEditor.keyHeader') }}
        th {{ $t('eventEditor.valueHeader') }}
