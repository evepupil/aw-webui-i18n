...
div
  ...
  div
    p
      | {{ $t('categoryBuilder.description1') }}

    p
      | {{ $t('categoryBuilder.description2') }}

    i18n-t(keypath="categoryBuilder.description3" tag="p")
      template(#settingsLink)
        router-link(:to="{ path: '/settings' }") {{ $t('categoryBuilder.settings') }}
  ...
  div.d-flex
    div.flex-grow-1
      ...
      div
        small {{ $t('categoryBuilder.hostnameLabel') }} {{ queryOptions.hostname }}
      div
        ...
        div.col.hover-highlight
          ...
          div(v-if="showing_events && showing_events[0] == word")
            table.table.table-sm.table-striped
              tr
                th {{ $t('categoryBuilder.titleHeader') }}
                th.text-right {{ $t('categoryBuilder.durationHeader') }}
  ...
  b-modal(id="appendRule" :title="$t('categoryBuilder.appendRuleModal')" @ok="handleOk" :ok-disabled="!valid")
    b-form(ref="form" @submit.stop.prevent="handleSubmit")
      ...
      b-form-group(:label="$t('categoryBuilder.wordLabel')")
        ...
        small
          div(v-if="validPattern" style="color: green") {{ $t('categoryBuilder.validPattern') }}
